%function [] = rp_HumanEvaII_test_and_generate_xml()
...
for use_cam=1:2
    ...
    savedir = 'F:\\test_sets\\image_descriptors\\HumanEva_II\\results\\original\\';

    % initialize dataset
    dset = dataset('HumanEvaII', 'Test');
    print(dset);

    % load database
    dirname = 'F:\\test_sets\\image_descriptors\\HumanEva\\';
    load(strcat(dirname, 'DB_V4II.mat'));
    ...
    db_pose = database(:, 274:513);
    clear database

    for SEQ = 1:length(dset)
        action = sprintf('HoG_V4II_%s_%s_%s', char(get(dset(SEQ, 1), 'SubjectName')), char(get(dset(SEQ, 1), 'ActionType')), char(get(dset(SEQ, 1), 'Trial')));

        % load test hogs
        filename = sprintf('%s_(c%d).mat', action, use_cam);
        load(filename)
        sq_hog = hog;
        clear hog
        ...
        % export to XML
        filename = sprintf('walking_%s_(c%d,n%d,t1)', action, use_cam, use_norm);
