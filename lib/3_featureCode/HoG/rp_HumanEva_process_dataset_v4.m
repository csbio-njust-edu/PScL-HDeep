%function [] = rp_HumanEva_process_dataset_v4()
% Processes the HumanEva-I data set and writes bounding boxes and HoG
...
%
% Syntax: 
%       [] = rp_HumanEva_process_dataset_v4();
%
% Variables:    
...
background_dir = 'F:\\test_sets\\image_descriptors\\HumanEva_I\\background_V4\\test\\';
...
for SEQ = 1:size(movie_dir, 1)
    if (movie_dir(SEQ).isdir == 1 && ~strcmp(movie_dir(SEQ).name, '.') && ~strcmp(movie_dir(SEQ).name, '..'))
        ...
        bg1_hsv = rgb2hsv(background(CAM).BackgroundMeans{1});
        bg2_hsv = rgb2hsv(background(CAM).BackgroundMeans{2});
        bg3_hsv = rgb2hsv(background(CAM).BackgroundMeans{3});
        ...
        for FRAME = 1:Nframes
            ...
            % S4 - cam 1
            %bg_img(286:296, :) = imerode(bg_img(286:296, :), strel([1 1 1 1]'));

            % S4 - cam 2
            %bg_img(290:304, :) = imerode(bg_img(290:304, :), strel([1 1 1 1 1 1]'));
            %bg_img(:, 534:640) = imerode(bg_img(:, 534:640), strel([1 1 1 1 1 1]));
            %bg_img(:, 1:139) = imerode(bg_img(:, 1:139), strel([1 1 1 1 1 1]));
            ...
            bg2_hsv_crop = imcrop(bg2_hsv, bounding_box);
            bg3_hsv_crop = imcrop(bg3_hsv, bounding_box);
            bg_img_crop_old = bg_img_crop;
            bg_img_crop(find((im_hsv(:, :, 2) - bg1_hsv_crop(:, :, 2) < 0) & abs(im_hsv(:, :, 2) - bg1_hsv_crop(:, :, 2)) <= val_thresh)) = 0;
            bg_img_crop(find((im_hsv(:, :, 2) - bg2_hsv_crop(:, :, 2) < 0) & abs(im_hsv(:, :, 2) - bg2_hsv_crop(:, :, 2)) <= val_thresh)) = 0;
            bg_img_crop(find((im_hsv(:, :, 2) - bg3_hsv_crop(:, :, 2) < 0) & abs(im_hsv(:, :, 2) - bg3_hsv_crop(:, :, 2)) <= val_thresh)) = 0;
            start_y = floor(4*y/5);
